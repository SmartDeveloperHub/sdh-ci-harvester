...
public class ServiceHandler extends Serviceable implements ResourceHandler {
	...
	private Service findContact(URI id) throws UnknownResourceException {
		Service contact=
	...
	public DataSet get(ResourceSnapshot resource) throws UnknownResourceException {
		...
		Service contact = findContact(serviceId);
		info("Retrieved service %s: %s",serviceId,contact);
		return ServiceMapper.toDataSet(contact);
	}
