 ...
 * #-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#
 *   Artifact    : org.smartdeveloperhub.harvesters.ci.backend:ci-backend-mem:0.2.0
 *   Bundle      : ci-backend-mem-0.2.0.jar
 * #-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#
 */
...
public class InMemoryTransactionManager implements TransactionManager {
	...
	void disposeTransaction(final InMemoryTransaction transaction) {
		final Transaction current=this.currentTransaction.get();
